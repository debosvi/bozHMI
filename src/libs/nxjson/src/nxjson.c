...
static int unicode_to_utf8(unsigned int codepoint, char *p, char **endp) {
    ...
    if (codepoint < 0x80)
        ...
        *p++ = 240 + codepoint / 262144, *p++ = 128 + codepoint / 4096 % 64, *p++ =
            128 + codepoint / 64 % 64, *p++ = 128 + codepoint % 64;
    else
        return 0;               // error
    *endp = p;
    return 1;
...
////////////////////////////////////////////////////////////////////////
static int nx_json_elems_count(const char *key) {
...
static void nx_json_build_keys(const char **keys, char *key) {
    register int i = 0, j = 0;
    register char *p = key;
    while (1) {
        if (key[j] == '\0')
...
const nx_json *nx_json_get(const nx_json * json, const char *key) {
    ...
    const char *keys[nx_json_elems_count(key) + 1];
    memset(keys, 0, sizeof(keys));

    //fprintf(stderr, "%s: key(%s)\n", __FUNCTION__, key);
    nx_json_build_keys(keys, dupkey);

    for (i = 0, js = json->child; js;) {
        //fprintf(stderr, "%s: try match key, expected(%s), current(%s)\n", __FUNCTION__, keys[i], js->key);
        if (js->key && !strcmp(js->key, keys[i])) {
            //fprintf(stderr, "%s: match key(%s)\n", __FUNCTION__, keys[i]);
            if (!keys[++i]) {
                //fprintf(stderr, "%s: no more keys to look for\n", __FUNCTION__);
                return js;
            }
            else {
                if (js->next)
                    ...
                    js = js->child;
                //fprintf(stderr, "%s: next look for (%p)\n", __FUNCTION__, js);
            }
        }
        else {
            js = js->next;
